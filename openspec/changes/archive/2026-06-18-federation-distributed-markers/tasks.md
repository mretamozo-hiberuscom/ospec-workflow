...
## Phase 8: WU7 — Hardening Fixes (`resilience-warning-001`, `risk-warning-symlink-001`)

> Follow-on hardening slice raised by the 4R reviewers (two advisory WARNINGs).
> Built on top of WU1–WU6 (`4efe753`, `f30ab07`, `06462da`, `fda2c5e`, `c3d3ff7`, `8656fc4`)
> on `feat/federation-distributed-markers`. Closes the data-loss-on-I/O-error gap in
> `readExistingMarker` and the lexical-only containment symlink-escape gap in `scanMemberMarkers`.

- [x] 8.1 [TEST-RED][Fix A] In `scripts/lib/federation-marker.test.js`, add failing tests for `readExistingMarker`/`enroll`: a transient non-ENOENT read error (simulated `EACCES`) on a HEALTHY existing marker makes `enroll` REJECT and leave the marker untouched (no overwrite / data loss); the marker path being a directory raises `EISDIR` and `enroll` rethrows leaving no `.tmp` behind. Keep a green regression that a present-but-unparseable (whitespace) marker IS rewritten (`status: written`) — the corrupt self-heal stays green.
- [x] 8.2 [TEST-RED][Fix B] In `scripts/lib/workspace-atlas.test.js` and `scripts/lib/federation-explore.test.js`, add failing tests that plant a REAL symlink inside the container (clean name, no `../`) pointing outside: `scanMemberMarkers` skips it with a warning and performs no out-of-tree read; `explore` never enrolls/writes a marker THROUGH the symlink outside the real container while still enrolling the legitimate in-root member. Use `t.skip()` if the OS cannot create symlinks.
- [x] 8.3 [Fix A] In `scripts/lib/federation-marker.js`, split `readExistingMarker` into a read phase and a parse phase: `ENOENT` on read → `null` (absent, write fresh); any other read error (`EACCES`/`EBUSY`/`EISDIR`/lock) → RETHROW so `enroll` aborts instead of overwriting; parse failure on present content → `null` (self-heal rewrite). Make the I/O-vs-parse distinction explicit.
- [x] 8.4 [Fix B] In `scripts/lib/workspace-atlas.js`, add the physical containment guard `isRealPathWithinRoot(containerRoot, candidateAbs)` (resolve with `fs.realpath`/`fs.lstat`; accept not-yet-existing first-enroll dirs via ENOENT; accept non-symlink entries; reject EXISTING symlinks escaping or dangling) and apply it in `scanMemberMarkers`'s discovery→use boundary (warn + skip, fail-open). Keep the lexical `isWithinRoot` `../`/absolute rejection intact (no `risk-critical-001` regression).
- [x] 8.5 [NO-REGRESSION] Run full `npm test` — all Phase 1–7 tests plus the new WU7 tests pass. Confirm exact counts and that the change stays additive (`parseAtlas` byte-identical; no existing test modified).
