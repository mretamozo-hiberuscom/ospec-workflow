...
- Q: How does the delegated `sdd-baseline` resolve the coordinator root to locate `federation-baseline-status.yaml` (upward traversal from `target_dir` vs an explicit parameter)? → A: Explicit `coordinator_root` parameter (user-approved). The orchestrator, which always knows its own root, passes `coordinator_root` via the same `## Parameters` injection mechanism C1 uses for `target_dir`. Upward traversal from `target_dir` is retained ONLY as a fallback when `coordinator_root` is absent. Rationale: this is layout-agnostic — it works for both sibling layouts (`target_dir: ../member`, where the coordinator is NOT an ancestor) and nested layouts, and it resolves the internal contradiction between the sibling-layout example and the upward-traversal resolution previously described in the delegation contract.
