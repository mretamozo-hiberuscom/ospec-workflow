...
then surface them with `AskUserQuestion` so the user can scope reviewer load and
...
2. **Unified Gate**: Scan the fresh domain-maps of all candidates. If `unified_gate.status` is not `'approved'`, present a single unified gate to the user via `AskUserQuestion`. Once approved, record the approval atomically in `federation-baseline-status.yaml`.
