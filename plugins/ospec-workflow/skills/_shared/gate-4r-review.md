...
3. **Escalate**: If any finding has severity `BLOCKER` or `CRITICAL`, surface it to the user via `AskUserQuestion` before closing the route. This is MANDATORY — findings at these severities MUST NOT be silently dropped. The route does NOT auto-halt; the user decides remediation.
