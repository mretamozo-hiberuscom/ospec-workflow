...
### Step 1 — Offer via AskUserQuestion

Preguntar al usuario si tiene documentos del proyecto (PDF, especificación funcional, arquitectura, etc.) que desee aportar como contexto. Esta consulta DEBE ocurrir antes de iniciar las preguntas de descubrimiento. Usar `AskUserQuestion` con:
...
2. **Si el servidor MCP NO está disponible**:
   - Detener el flujo de ingesta y presentar un gate interactivo al usuario mediante `AskUserQuestion` con las siguientes opciones de remediación:
...
6. If active unresolved gaps exist, trigger a Q&A gate via `AskUserQuestion` (or return `status: blocked` with `question_gate`) to ask the user how to resolve them. Options must include: assign to a member, defer capability, or create a new member directory. Record user resolutions under `approvals` in `state.yaml` and `gaps_resolutions` in `openspec/config.yaml`.
7. Create or update:
