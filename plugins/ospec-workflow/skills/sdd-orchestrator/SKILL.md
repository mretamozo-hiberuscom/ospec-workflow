...
Whenever a blocking user decision is resolved through `AskUserQuestion`, persist a compact approval entry under:
...
When the user invokes `/sdd-new`, `/sdd-ff`, `/sdd-continue`, or `/sdd-lite` (or an equivalent natural-language request, e.g. "haceme un SDD para X" / "do SDD for X") for the first time in a session, use `AskUserQuestion` to ask which execution mode they prefer:
...
3. Use `AskUserQuestion` to ask whether to continue, stop, or provide adjustment feedback.
...
2. **On `status: blocked` with `question_gate`**: call `AskUserQuestion` with the `question_gate` payload; wait for all answers; relaunch `sdd-clarify` with the answers; record `state.yaml` `status: blocked` and `blocking_questions` while waiting. On relaunch success, go to step 1.
...
1. Intercept the block and call `AskUserQuestion` with the gap resolution options.
...
When the orchestrator receives `status: blocked` with `question_gate`, it MUST call `AskUserQuestion`, wait for the answer, and then relaunch or route the phase with the user's answer.

When the orchestrator receives `status: blocked` with only `next_question`, it MUST convert it to a single `AskUserQuestion` freeform question.
